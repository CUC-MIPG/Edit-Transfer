...
from torchvision import transforms as transforms2
...
class StableDiffusion:
    ...
    def __init__(
        ...
        if self.is_flux or self.is_v3 or self.is_auraflow or isinstance(self.noise_scheduler,
            ...
            with torch.no_grad():
                ...
                for i in tqdm(range(len(image_configs)), desc=f"Generating Images", leave=False):
                    ...
                    if self.decorator is not None:
                        ...
                        unconditional_embeds.text_embeds = self.decorator(unconditional_embeds.text_embeds,
                                                                          is_unconditional=True)
    ...
    def get_latent_noise(
        ...
        # mask = self.mask.to(self.device_torch, dtype=torch.float16).detach()
        # # # scale down to the size of the latents, mask multiplier shape(bs, 1, width, height), noisy_latents shape(bs, channels, width, height)
        # mask = torch.nn.functional.interpolate(
        #     mask, size=(noise.shape[2], noise.shape[3])
        # )
        # print("noise", noise.size())
        # print("mask", self.mask.size())
        #noise = noise * mask
    ...
    def add_noise(
        ...
        for idx in range(original_samples.shape[0]):
            noisy_latents = self.noise_scheduler.add_noise_mask2(original_samples_chunks[idx], noise_chunks[idx],
                                                       timesteps_chunks[idx],mask)
    ...
    def encode_images(
        ...
        # img = (latents/ self.vae.config.scaling_factor) + self.vae.config.shift_factor
        # img = self.vae.decode(img.cpu(), return_dict=False)[0]
        # to_pil = transforms2.ToPILImage()
        # image = to_pil(img[0])
        #
        # # 保存图像

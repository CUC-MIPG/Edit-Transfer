...
class SDTrainer(BaseSDTrainInpaintProcess):
    ...
    def calculate_loss(
        ...
        if self.train_config.correct_pred_norm or (
                self.train_config.inverted_mask_prior and prior_pred is not None and has_mask):
        ...
        if self.dfe is not None:
            if self.dfe.version == 1:
                ...
                for i in range(len(target_feature_list)):
                    dfe_loss += torch.nn.functional.mse_loss(pred_feature_list[i], target_feature_list[i],
                                                             reduction="mean")
        ...
        if loss_target == 'source' or loss_target == 'unaugmented':
            ...
            if loss_target == 'source':
                ...
                with torch.no_grad():
                    unaugmented_latents = self.sd.encode_images(batch.unaugmented_tensor).to(self.device_torch,
                                                                                             dtype=dtype)
        ...
        if self.train_config.train_turbo:
            ...
            mask_multiplier = torch.nn.functional.interpolate(mask_multiplier, size=(pred.shape[2], pred.shape[3]),
                                                              mode='nearest')
        ...
        if self.adapter is not None and hasattr(self.adapter,
                                                "additional_loss") and self.adapter.additional_loss is not None:
